...
export type EventType = 'policeRaid' | 'betrayal' | 'rivalAttack' | 'criminalCaught' | 'soldierDesertion' | null;
...
const INITIAL_BUILDINGS: Building[] = [
  { id: 'bld-1', name: 'Golden Dragon Noodles', type: 'Noodle Shop', baseRevenue: 500, heatGen: 1, isOccupied: false, assignedOfficerId: null, inactiveUntilDay: null, isIllicit: false, foodProvided: 30, entertainmentProvided: 5, upgraded: false },
  { id: 'bld-2', name: 'Lucky Fortune Parlor', type: 'Mahjong Parlor', baseRevenue: 800, heatGen: 3, isOccupied: false, assignedOfficerId: null, inactiveUntilDay: null, isIllicit: true, foodProvided: 0, entertainmentProvided: 40, upgraded: false },
  { id: 'bld-3', name: 'Harbor Warehouse #7', type: 'Warehouse', baseRevenue: 400, heatGen: 2, isOccupied: false, assignedOfficerId: null, inactiveUntilDay: null, isIllicit: false, foodProvided: 0, entertainmentProvided: 0, upgraded: false },
  { id: 'bld-4', name: 'Neon Paradise Club', type: 'Nightclub', baseRevenue: 600, heatGen: 4, isOccupied: false, assignedOfficerId: null, inactiveUntilDay: null, isIllicit: true, foodProvided: 10, entertainmentProvided: 60, upgraded: false },
  { id: 'bld-5', name: 'Jade Tiger Kitchen', type: 'Noodle Shop', baseRevenue: 450, heatGen: 1, isOccupied: false, assignedOfficerId: null, inactiveUntilDay: null, isIllicit: false, foodProvided: 35, entertainmentProvided: 5, upgraded: false },
  { id: 'bld-6', name: 'OCTB Precinct', type: 'Police Station', baseRevenue: 0, heatGen: -15, isOccupied: false, assignedOfficerId: null, inactiveUntilDay: null, isIllicit: false, foodProvided: 0, entertainmentProvided: 0, upgraded: false },
];
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  assignOfficer: (officerId: string, buildingId: string) => {
    const state = get();
    if (state.currentPhase !== 'morning') return; // Can only assign in morning
      ...
      const updatedOfficers = state.officers.map(o => {
        ...
        if (building.baseRevenue > 600) {
          const newRelationships = o.relationships.map(r => 
            r.targetId === officerId ? { ...r, respect: Math.max(-100, r.respect - 5) } : r
          );
    ...
    set((state) => {
      ...
      return {
        officers: state.officers.map(o =>
          o.id === officerId ? { ...o, assignedBuildingId: null, daysAssigned: 0 } : o
        ),
        buildings: state.buildings.map(b =>
    ...
    set((state) => {
      ...
      switch (choice) {
        ...
        case 'punish':
          updates.officers = state.officers.map(o => {
            if (o.id === betrayerId) {
              return { ...o, isBetraying: false, loyalty: Math.max(0, o.loyalty - 30), energy: Math.max(0, o.energy - 40) };
            }
      ...
      switch (choice) {
        ...
        case 'enslave':
          ...
          const newSlave: StreetSoldier = {
            ...
            loyalty: 20,
            needs: { food: 50, entertainment: 30, pay: 100 }, // Doesn't need pay
            skill: 20,
            isDeserting: false,
          };
    ...
    set((state) => {
      ...
      switch (choice) {
        case 'fight':
          if (totalStrength > rival.strength) {
            ...
            updates.rivals = state.rivals.map(r =>
              r.id === rivalId ? { ...r, relationship: r.relationship - 30, strength: Math.max(10, r.strength - 20) } : r
            );
          } else {
            // Defeat
        ...
        case 'negotiate':
          ...
          updates.rivals = state.rivals.map(r =>
            r.id === rivalId ? { ...r, relationship: r.relationship + 10 } : r
          );
          break;
        case 'retreat':
    ...
    set((state) => {
      ...
      switch (action) {
        case 'trade':
          ...
          updates.rivals = state.rivals.map(r =>
            r.id === rivalId ? { ...r, hasTradeAgreement: true, relationship: r.relationship + 15 } : r
          );
          break;
        case 'alliance':
          ...
          updates.rivals = state.rivals.map(r =>
            r.id === rivalId ? { ...r, hasAlliance: true, relationship: r.relationship + 25 } : r
          );
          break;
        case 'turfWar':
          // Queue a rival attack event
          updates.pendingEvents = [...state.pendingEvents, { type: 'rivalAttack' as EventType, data: { rivalId } }];
          updates.rivals = state.rivals.map(r =>
            r.id === rivalId ? { ...r, relationship: r.relationship - 40 } : r
          );
          break;
      }
    ...
    set((state) => {
      ...
      if (building.type === 'Warehouse') {
        return {
          ...
          buildings: state.buildings.map(b =>
            b.id === buildingId
              ? { ...b, type: 'Counterfeit Lab' as BuildingType, baseRevenue: 1500, heatGen: 12, isIllicit: true, upgraded: true, name: b.name + ' (Lab)' }
              : b
          ),
        };
      }
...
function processNightEvents(state: GameState): { updates: Partial<GameState>; events: { type: EventType; data: any }[] } {
  ...
  state.officers.forEach(officer => {
    if (officer.loyalty < 40 && officer.daysAssigned === 0) {
      ...
      if (avgRespect < 30 && Math.random() < 0.2) {
        events.push({ type: 'betrayal', data: { officerId: officer.id, officerName: officer.name } });
      }
    }
  });
  ...
  state.rivals.forEach(rival => {
    if (rival.relationship < -30 && !rival.hasAlliance && Math.random() < 0.15) {
      events.push({ type: 'rivalAttack', data: { rivalId: rival.id, rivalName: rival.name } });
    }
  });
