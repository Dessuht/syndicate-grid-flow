...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  dismissEvent: () => {
    ...
    if (state.pendingEvents.length > 0) {
      // If there are pending events, process the next one
      set((state) => {
        const [nextEvent, ...rest] = state.pendingEvents;
        return {
          activeEvent: nextEvent.type,
      ...
      // If this is the last event, clear it
      set({ activeEvent: null, eventData: null });
      
      // If the phase was interrupted (i.e., not morning), resume the phase cycle.
  ...
  resolveStreetWar: (choice: 'bribe' | 'fight') => {
    // This function is now redundant as handleStreetWarChoice handles the resolution.
    // Keeping it here for interface compatibility but directing logic to handleStreetWarChoice.
    get().handleStreetWarChoice(choice);
  },
