...
export const GlobalMap = () => {
  const { cash, intel, rivals, soldiers, activeDiplomacy, initiateDiplomacy, confirmDiplomacy, cancelDiplomacy, purchaseIntel, buildings, homeDistrictLeaderId, syndicateMembers, processRacketCycle, homeDistrictRevenue, homeDistrictHeat, territoryFriction, territoryInfluence, spendIntelToReduceFriction, spendIntelToScout } = useGameStore();
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-lg bg-neon-purple/10 border border-neon-purple/30">
            <FaGlobe className="w-5 h-5" style={{ color: 'hsl(var(--neon-purple))' }} />
          </div>
          <div>
        ...
        <Button variant="outline" onClick={() => purchaseIntel(500)} disabled={cash < 500} className="gap-2">
          <FaShoppingCart className="w-4 h-4" /> Buy Intel ($500)
        </Button>
      </div>
        ...
        {rivals.map((rival, index) => (
          <div key={rival.id} className="flex items-center gap-2">
            <span 
              ...
              style={{ 
                backgroundColor: rival.isActiveConflict ? '#FF0000' : 
                                rival.hasAlliance ? '#44FF44' : 
                                rival.relationship < -20 ? '#FF4444' : 
                                GANG_COLORS[`rival-${index + 1}`] 
              }} 
            />
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-4 min-h-0 overflow-hidden">
        {/* Map - larger area */}
        <div className="flex-1 lg:flex-[2] relative rounded-lg overflow-hidden border border-border" style={{ minHeight: '350px', zIndex: 0 }}>
          <div ref={mapContainer} className="absolute inset-0" style={{ background: '#1a1a2e', zIndex: 0 }} />
        </div>
        ...
        <div className="lg:w-80 space-y-4 overflow-auto">
          <AnimatePresence mode="wait">
            {selectedPlayerTerritory ? (
              ...
              >
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  {selectedRivalData.isScouted && (
                    <div className="flex items-center gap-1.5">
                      <FaEye className="w-4 h-4 text-neon-amber" />
                      <span className="text-sm text-neon-amber">Scouted</span>
                    </div>
                  )}
                  {selectedRivalData.isActiveConflict && (
                    <div className="flex items-center gap-1.5">
                      <FaExclamationTriangle className="w-4 h-4 text-neon-red animate-pulse" />
                ...
                <div className="space-y-2 mb-4">
                  <Button 
                    ...
                    disabled={intel < 20}
                  >
                    <FaBolt className="w-4 h-4" /> Reduce Friction (20 Intel)
                  </Button>
                  {!selectedRivalData.isScouted && (
                    <Button 
                      variant="outline" 
                      ...
                      disabled={intel < 50}
                    >
                      <FaEye className="w-4 h-4" /> Scout Territory (50 Intel)
                    </Button>
                  )}
                </div>
              ...
              >
                <FaMapMarkerAlt className="w-8 h-8 mx-auto mb-3 text-muted-foreground/50" />
                <p className="text-sm text-muted-foreground">Click a territory marker on the map</p>
              </motion.div>
            )}
          </AnimatePresence>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {rival.isActiveConflict && (
                      <FaExclamationTriangle className="w-3 h-3 text-neon-red animate-pulse" />
                    )}
                  </div>
                  <span className={cn(
                    "text-xs font-medium",
                    rival.relationship > 0 ? 'text-neon-green' : 
                    rival.relationship < 0 ? 'text-neon-red' : 
          ...
          >
            ...
            >
              ...
              <div className="flex gap-2">
                ...
                >
                  <FaCheck className="w-4 h-4 mr-1" /> Confirm
                </Button>
              </div>
            </motion.div>
          </motion.div>
